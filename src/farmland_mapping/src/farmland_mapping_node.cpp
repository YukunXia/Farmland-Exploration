...
//   assert(world_size > 0.0f);tf tree missing frame
...
//       Eigen::RowMajor>>( occupancy_grid.data.data(),
//       occupancy_grid.info.height, occupancy_grid.info.width);
...
void gazeboStatesCallback(const gazebo_msgs::ModelStatesConstPtr msg) {
  ...
  // tf::poseMsgToEigen(msg->pose.at(robot_state_index),
  // world__eigenT__base_link); world__eigenT__velodyne =
  //     world__eigenT__base_link * base_link__eigenT__velodyne;

  static tf::TransformBroadcaster br;

  ros::Time curr_time = ros::Time::now();
  if (curr_time.toSec() >= robot_odometry_tf_last_time + 0.01) {
  ...
  // if (robot_odometry_broadcaster &&
  //     curr_time.toSec() >= robot_odometry_tf_last_time + 0.01) {
  //   robot_odometry_tf.header.frame_id = "world";
  //   robot_odometry_tf.header.stamp = curr_time;
  //   robot_odometry_tf.child_frame_id = "base_link";
  //   robot_odometry_tf.transform.rotation =
  ...
  //   robot_odometry_tf.transform.translation.z =
  //       msg->pose.at(robot_state_index).position.z;
  //   robot_odometry_broadcaster->sendTransform(robot_odometry_tf);

  //   robot_odometry_tf_last_time = curr_time.toSec();
  // }

  // robot_odometry_msg.header.frame_id = "world";
  // robot_odometry_msg.header.stamp = ros::Time::now();
  // robot_odometry_msg.child_frame_id = "base_link";
  // robot_odometry_msg.pose.pose = msg->pose.at(robot_state_index);
...
int main(int argc, char **argv) {
  ...
  // robot_odometry_broadcaster =
  //     std::make_unique<tf2_ros::TransformBroadcaster>();
  // velodyne_point_cloud_sub =
  //     nh.subscribe("/points", 100, velodynePointCloudCallback);
  gazebo_states_sub =
      nh.subscribe("/gazebo/model_states", 100, gazeboStatesCallback);
  // robot_odometry_pub =
  //     nh.advertise<nav_msgs::Odometry>("/robot_odometry_to_world", 100);
