...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle nh;
    ros::ServiceClient client = nh.serviceClient<nav_msgs::GetPlan>("/planner/planner/make_plan");
    ...
    if (client.call(srv)) {
        if (srv.response.plan.poses.size() > 0) {
            ROS_INFO("A plan is made, and its length is %ld", srv.response.plan.poses.size());
